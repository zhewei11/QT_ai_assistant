...
class FixedLSTM(tf.keras.layers.LSTM):
    """
    LSTM class wrapper to intercept and remove the 'time_major' argument 
    from the saved model configuration before passing it to the Keras 
    constructor, resolving ValueError in newer Keras versions.
    """
    @classmethod
    def from_config(cls, config):
        # The 'config' dictionary contains the saved parameters for the layer.
        # Pop the problematic 'time_major' parameter if it exists.
        config.pop('time_major', None) 
        config.pop('zero_output_for_mask', None) 
        # Call the parent class's from_config with the cleaned config
        return super(FixedLSTM, cls).from_config(config)
...
#            orig_peaks = processing.resample_ann(resampled_pos, filtered_peaks)
    ...
    def remove_close(self, peaks, peak_probs, threshold_ms=200, verbose=False):
        ...
        while close_probs.shape[0] > 0:
            ...
            if ok_peaks.size == 0:
                ok_peaks = np.array([max_peak])

            elif max_peak < ok_peaks[0]:
